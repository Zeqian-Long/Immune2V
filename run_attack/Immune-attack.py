...
def run_attack(pipe, image, h, w, num_frames, prompt_emb_src, prompt_emb_tgt, image_emb_src, image_emb_tgt, latents_list, num_steps=400, epsilon=20.0 / 255 * 2, step_size=2.0 / 255 * 2):
    ...
    for step in pbar:
        ...
        # Enc Loss
        D = image_emb_adv["y"][0, 4:] - image_emb_tgt[0]
        enc_loss = torch.sqrt((D ** 2).sum(dim=(0, 2, 3))).sum()
        ...
        noise_pred = prompt_img_sem_loss(pipe.dit, adv_latents, timestep=timestep, **prompt_emb_src, **image_emb_adv)
        noise_pred_tar = prompt_img_sem_loss(pipe.dit, adv_latents, timestep=timestep, **prompt_emb_tgt, **image_emb_src)  
